...
void send_all(int except) {  // разослать всем клиентам сообщение
    for (int i = 0; i <= max; i++)
...
int main(int ac, char **av) {
    ...
    max = serverSock; // всегда обновлять max
    FD_SET(serverSock, &active); // всегда сетить нoвые fd
    ...
    while (1) {
        ...
        for (int s = 0; s <= max; s++) {
            ...
            if (FD_ISSET(s, &readyRead) && s == serverSock) { // пришло сообщение на сервер
                int clientSock = accept(serverSock, (struct sockaddr *)&addr, &addr_len);
                if (clientSock < 0)
                continue ;

                max = (clientSock > max) ? clientSock : max; // всегда обновлять max
                id[clientSock] = next_id++;
                FD_SET(clientSock, &active); // всегда сетить нoвые fd
            ...
            if (FD_ISSET(s, &readyRead) && s != serverSock) { { // пришло сообщение не от сервера
